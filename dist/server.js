...
async function getPrice(params) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Binance API 错误: ${response.status} ${response.statusText}. ${JSON.stringify(errorData)}`);
        }
 ...
 * 获取 Binance 24小时统计数据
 */
async function get24hStats(params) {
    const { symbol, quote } = params;
    // 转换为 Binance 格式：BTC -> BTCUSDT
    const binanceSymbol = `${symbol.toUpperCase()}${quote.toUpperCase()}`;
 ...
 */
async function getOhlcv(params) {
...
function analyzeTrend(klines) {
    ...
    // 计算综合趋势强度（加权平均）
    trendStrength = Math.max(-1, Math.min(1, priceFactor * 0.4 + // 价格变化权重 40%
 ...
 * 整合价格、24小时统计、K线趋势分析
 */
async function getSymbolOverview(params) {
    const { symbol, quote } = params;
    const binanceSymbol = `${symbol.toUpperCase()}${quote.toUpperCase()}`;
    try {
        // 并行获取24小时统计和K线数据
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                throw new McpError(ErrorCode.InvalidParams, `参数验证失败: ${error.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join(", ")}`);
            }
